...
<!-------------------이하 콘텐츠--------------- -->
<form name = "sel_list" method="post" action="rank_changer.jsp">
	<table border="1" style="width:500px; margin:auto;">
		<tr>
			<td colspan="3" style="height:30px; text-align:center; font-size:20px;">판매 목록</td>
		</tr>
	
		<%
		sql  = "select * from sellist where mnum = '"+mnum+"' order by slnum desc" ; 
		dbms.OpenQuery(sql);
		...
		for(int i = 0 ; i<data.size(); i++)
		{
			...
			String gstate   = dbms.GetValue("gstate");		//제품의 판매여부(a생존 / E평가 / d판매완료)
			String mdelnum	= dbms.GetValue("mdelnum");		//판매자의 거래수
		...
		<tr>
			<td style="width:30%; height:70px; <% if(gstate.equals("D") || gstate.equals("E") ) {%>background-color: #808080;<%}%>" >
			...
			<% if(gstate.equals("D") || gstate.equals("E") )
			...
			<td style="width:15%; text-align:center; background-color: #808080;">
				<div> <input type="button" value="연락" onClick="location.href='tel.jsp?tnum=<%=tnum %>'"> </div><br>			
			...
			<td style="width:15%; text-align:center;">
				<div> <input type="button" value="연락" onClick="location.href='tel.jsp?tnum=<%=tnum %>'"> </div><br>
				<div> <input type="button" value="삭제" onClick="location.href='delete_list.jsp?gnum=<%=gnum %>&gmnum=<%=gmnum %>&type=<%=type%>'"> </div><br>
				<div> <input type="button" id = "deal_done" onclick="location.href='point_set.jsp?gnum=<%=gnum%>'" value="거래완료"> </div>		
			</td>
			<%}	%>		
		</tr>		
		<%
